...
def create_qa_chain(vector_db, use_local_llm=True):
    """Создание цепочки вопрос-ответ с выбранной моделью"""
    if use_local_llm:
        ...
        print("Использование OpenAI API...")
        # Проверяем наличие ключа OpenAI
        if not os.getenv("OPENAI_API_KEY"):
            raise ValueError("API ключ OpenAI не найден в .env файле. Добавьте OPENAI_API_KEY=ваш-ключ в .env файл")
...
Контекст содержит документы пользователя о различных темах.
...
6. Если нет релевантной информации, честно скажи "В документах нет информации по этому вопросу"
...
"""
    ...
    # Создаем RAG цепочки с указанной моделью
